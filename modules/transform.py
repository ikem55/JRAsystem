...
class Transform(object):
    ...
    def choose_race_df_columns(self, race_df):
        race_df = race_df[['RACE_KEY', 'KAISAI_KEY', 'NENGAPPI', '発走時間', '距離', '芝ダ障害コード', '右左',
                           '内外', '種別', '条件', '記号', '重量','頭数', 'コース', '開催区分', 'データ区分', 'WIN5フラグ', 'target_date', '曜日', '場名',
    ...
    def choose_raceuma_df_columns(self, raceuma_df):
        raceuma_df = raceuma_df[['RACE_KEY', 'UMABAN', 'NENGAPPI', '血統登録番号', '脚質', '距離適性', '上昇度', 'ローテーション', '基準オッズ', '基準人気順位', '基準複勝オッズ',
                                 '基準複勝人気順位', '総合情報◎', '総合情報○', '総合情報▲', '総合情報△',
                                 '総合情報×', '調教矢印コード', '厩舎評価コード', '騎手期待連対率', '激走指数', '蹄コード', '重適正コード', '騎手名',
                                 ...
                                 'ZENSO5_RACE_KEY', '枠番', '総合印', 'ＩＤＭ印', '情報印', '騎手印', '厩舎印', '調教印', '激走印',  '騎手コード',
                                 '調教師コード','道中順位', '後３Ｆ順位', 'ゴール順位',
                                 '展開記号', '距離適性２', '枠確定馬体重', '枠確定馬体重増減', '性別コード', '馬主名',
                                 '馬主会コード', '激走順位', 'LS指数順位', 'テン指数順位', 'ペース指数順位', '上がり指数順位', '位置指数順位', '騎手期待単勝率',
                                 '騎手期待３着内率', '輸送区分', '馬スタート指数', '馬出遅率', '万券印',
                                 '放牧先', '放牧先ランク', '厩舎ランク', 'target_date',
                                 '調教コースコード', '追切種類', '追い状態', '乗り役',
                                 '調教タイプ', '調教コース種別', '調教距離', '調教重点', '調教量評価', '仕上指数変化', '調教評価', '父馬産駒芝連対率', '父馬産駒ダ連対率',
                                 '父馬産駒連対平均距離', '母父馬産駒芝連対率', '母父馬産駒ダ連対率', '母父馬産駒連対平均距離', 'IDM', '騎手指数', '情報指数', '総合指数', '人気指数',
                                 '調教指数', '厩舎指数', 'テン指数', 'ペース指数', '上がり指数', '位置指数', '万券指数','追切指数',
                                 '仕上指数', '馬番グループ', '基準人気グループ']].copy()
        # '特定情報◎', '特定情報○', '特定情報▲', '特定情報△', '特定情報×',  'ブリンカー', 'ゴール内外','後３Ｆ内外','道中内外',
        #  'テンＦ指数', '中間Ｆ指数', '終いＦ指数', 'クラスコード','芝適性コード', 'ダ適性コード','馬記号コード','道中差', '後３Ｆ差','ゴール差',
        # '獲得賞金', '収得賞金', '条件クラス', 'ペース予想', '取消フラグ',
        # '降級フラグ', '激走タイプ', '入厩何走目', '入厩年月日', '入厩何日前',
        # '休養理由分類コード', '芝ダ障害フラグ', '距離フラグ', 'クラスフラグ', '転厩フラグ', '去勢フラグ', '乗替フラグ',
        # 'CID調教素点', 'CID厩舎素点', 'CID素点', 'CID', 'LS指数', 'LS評価',
        # 'EM', '厩舎ＢＢ印', '厩舎ＢＢ◎単勝回収率', '厩舎ＢＢ◎連対率', '騎手ＢＢ印', '騎手ＢＢ◎単勝回収率', '騎手ＢＢ◎連対率', '調教曜日', '調教回数',
        # '調教Ｆ', 'テンＦ', '中間Ｆ', '終いＦ', '併せ結果', '併せ追切種類', '併せ年齢', '併せクラス',
        # '調教コース坂', '調教コースW', '調教コースダ', '調教コース芝', '調教コースプール', '調教コース障害', '調教コースポリ',
        # 'raceuma_before_taikei_0', 'raceuma_before_taikei_1','raceuma_before_taikei_2', 'raceuma_before_tokki_0', 'raceuma_before_tokki_1',
        # , 'ＪＲＡ成績', '交流成績', '他成績', '芝ダ障害別成績', '芝ダ障害別距離成績', 'トラック距離成績',
        ...
        raceuma_df = raceuma_df.astype({'脚質': int, '距離適性': int, '上昇度': int, '調教矢印コード': int,'厩舎評価コード': int, '激走指数': int,'蹄コード': int, '重適正コード': int,
                                        '負担重量': int, '見習い区分': int, '枠番': int, '総合印': int, 'ＩＤＭ印': int,
                                        '情報印': int, '騎手印': int, '厩舎印': int, '調教印': int, '激走印': int,
                                        '展開記号': int, '距離適性２': int, '枠確定馬体重': int, '枠確定馬体重増減': int,
                                        '性別コード': int, '馬主会コード': int, '輸送区分': int, '万券印': int,
                                        '追切種類': int, '追い状態': int, '乗り役': int,
                                        '調教タイプ': int, '調教コース種別': int, '調教距離': int, '調教重点': int, '仕上指数変化': int, '調教評価': int})
        # '併せ結果': int, '併せ追切種類': int, '併せ年齢': int,'条件クラス': int,'ブリンカー': int, 'クラスコード': int, '芝適性コード': int, 'ダ適性コード': int,
        # '道中内外': int, '後３Ｆ内外': int, 'ゴール内外': int, '馬記号コード': int,
        # '降級フラグ': int, '休養理由分類コード': int, '芝ダ障害フラグ': int, '距離フラグ': int, 'クラスフラグ': int, '転厩フラグ': int, '去勢フラグ': int, '乗替フラグ': int, '入厩何走目': int, '入厩何日前': int,
        # 'CID': int, 'EM': int, '厩舎ＢＢ印': int, '騎手ＢＢ印': int, '調教回数': int, '調教曜日': int, '放牧先ランク': int, '調教師所属': int, '激走タイプ': int, '調教コースコード': int,'取消フラグ': int,
        return raceuma_df
    ...
    def encode_raceuma_result_df(self, raceuma_df, dict_folder):
        ...
        raceuma_df = mu.hash_eoncoding(raceuma_df, hash_tokki_column, 3, hash_tokki_dict_name, dict_folder)
        ...
        raceuma_df = mu.hash_eoncoding(raceuma_df, hash_bagu_column, 2, hash_bagu_dict_name, dict_folder)
        ...
        raceuma_df = mu.hash_eoncoding(raceuma_df, hash_taikei_column, 2, hash_taikei_dict_name, dict_folder)
        return raceuma_df
    ...
    def choose_raceuma_result_df_columns(self, raceuma_df):
        raceuma_df = raceuma_df[['RACE_KEY', 'UMABAN', 'target_date', 'KYOSO_RESULT_KEY', '血統登録番号', 'NENGAPPI', '距離', '芝ダ障害コード',
                                  '頭数', '着順','確定単勝オッズ', '確定単勝人気順位', '不利',
                                 '1(2)着タイム差',  '確定複勝オッズ下', 'コーナー順位１', 'コーナー順位２', 'コーナー順位３', 'コーナー順位４',
                                 'レース脚質', 'テン指数結果順位', '上がり指数結果順位', 'ペース指数結果順位',
                                 'ラップ差１ハロン', '騎手コード',
                                 'コーナー順位１率', 'コーナー順位２率', 'コーナー順位３率', 'コーナー順位４率',
                                 '芝', '外', '重', '軽', 'TRACK_BIAS_ZENGO', 'TRACK_BIAS_UCHISOTO',
                                 'course_cluster', 'ru_cluster', 'fa_1', 'fa_2', 'fa_3',
                                 'fa_4', 'fa_5', 'ＩＤＭ結果', 'テン指数結果', '上がり指数結果', 'ペース指数結果', 'レースＰ指数結果', '馬体重', '非根幹', '距離グループ', '先行率',
                                 '人気率', '着順率', '追込率', '平均タイム', '平坦コース', '急坂コース', '好走', '内枠', '外枠', '中山', '東京', '上がりかかる', '上がり速い', 'ダート道悪',
                                 '前崩れレース', '先行惜敗', '前残りレース', '差し損ね', '上がり幅小さいレース', '内枠砂被り',
                                 'raceuma_result_tokki_1','raceuma_result_tokki_2','raceuma_result_tokki_0', 'raceuma_result_bagu_1', 'raceuma_result_bagu_0',
                                 'raceuma_result_taikei_1', 'raceuma_result_taikei_0']].copy()
        #  '内外', '馬場状態','後３Ｆ先頭差','馬体重増減',  '前３Ｆ先頭差',
        # '前３Ｆタイム', '後３Ｆタイム','種別', '条件', '記号', '重量', 'タイム', '素点', '馬場差','４角コース取り', 'IDM', '調教師コード', '右左',
        # '特記コード１', '特記コード２', '特記コード３', '特記コード４', '特記コード５', '特記コード６',
        # 'ラップ差４ハロン', 'ラップ差３ハロン','ラップ差２ハロン',
        # 'ペースアップ位置', 'RAP_TYPE', '芝種類', '草丈', '転圧','凍結防止剤', 'ハロン数',
        # '１ハロン平均_mean','ＩＤＭ結果_mean', 'テン指数結果_mean', '上がり指数結果_mean', 'ペース指数結果_mean', '前３Ｆタイム_mean', '後３Ｆタイム_mean',
        # '斤量',
        # 'コーナー順位１_mean', 'コーナー順位２_mean', 'コーナー順位３_mean', 'コーナー順位４_mean', '前３Ｆ先頭差_mean', '後３Ｆ先頭差_mean', '追走力_mean', '追上力_mean', '後傾指数_mean',
        # '１ハロン平均_std', '上がり指数結果_std', 'ペース指数結果_std', '場コード',
        raceuma_df = raceuma_df.astype({'芝ダ障害コード': int, 'レース脚質': int})
        # '右左': int, '内外': int, '馬場状態': int, '種別': int, '記号': int, '重量': int, '４角コース取り': int,  '場コード': int,
        # '特記コード１': int, '特記コード２': int, '特記コード３': int, '特記コード４': int,
        # '特記コード５': int, '特記コード６': int, 'ペースアップ位置': int, 'RAP_TYPE': int, '芝種類': int, '転圧': int, '凍結防止剤': int,
        return raceuma_df
